...
- **内嵌多主服务发现，无独立 meta 节点** —— 每个 host 都在进程内运行服务发现;
  当前 master 是按主机名排序最小的 `max_masters`(默认 3)个存活 host,某个 master
  挂掉会自动由下一个 host 顶上,节点数不足时全员皆 master(无单点故障)。把
  `discovery_addr` 配成一个或多个引导 seed(`host:port`,逗号分隔)即可;节点向其
  注册、心跳并拉取实时成员列表。这里不存放任何数据,也不存放任何元数据。
- **一致性哈希目录（DHT）** —— 映射
