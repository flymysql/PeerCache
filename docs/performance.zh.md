...
**0.41 TB/s**。(满负载多进程下单卡跑到了 **89 GB/s**——上面 16-QP 的 `ib_read_bw`
只是保守的单卡参考值,不是硬上限。)
...
方式(8 个 rank,各绑本地网卡)。这里:8 卡 × 每卡 8 个读进程,128 KiB 页。

<figure markdown>
  ![每卡吞吐](assets/perf/per_card.png)
  <figcaption>每张卡的 GET 吞吐;8 张合计 413.1 GB/s(≈ 3.3 Tbps)。</figcaption>
</figure>
...
聚合远超单进程(147 → 413 GB/s),且单卡在此跑到了 **89 GB/s**,所以**不受网卡限制**——
瓶颈是主机内存带宽 / PCIe 以及**不均衡**(两张卡只有 ~25 GB/s,其余 49–89)。本机上网卡
1–4 在 NUMA node 0、5–8 在 node 1(远端节点距离 32,本地 10),没绑核的读进程可能落到错误的
节点、付出跨 NUMA 代价。用
...
实测(单进程,8 rail,1 MiB 页,读进显存):**49.5 GB/s,100% 命中**,而 host 接收为
140 GB/s。这是**单 GPU 的 PCIe 瓶颈**——8 条 rail 全写进同一块 GPU,共享它的 PCIe 链路
(Gen5 x16 约 50 GB/s)。真实 SGLang TP=N 部署里每个 rank 读进**自己的** GPU,所以 GPUDirect
随 GPU 数量线性放大(≈ N × 单 GPU PCIe 带宽),不会被单条链路限制。
